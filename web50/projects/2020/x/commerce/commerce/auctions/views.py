...
def index(request):
    ...
    auctions = []
    auctions_watchlisted = [] # Lista de subastas en watchlist

    if user.is_authenticated:
        watchlist_count = len(utils.get_watchlist(user))
        #obtenemos las subastas que tiene el usuario en su lista de seguimiento
        auctions_watchlisted = utils.get_user_watchlist_auctions(user,is_active=True)

    #otbenemos todas las subastas activas existentes
    ...
    # checkeamos las subastas que terminaron para actualizar 
    ended_auctions = utils.get_auctions_by(is_active=True,end_date__lte=current_time.date(),end_time__lte=current_time.time())
    for auction in ended_auctions:
        highest_bid = auction.bid.order_by('-amount').first()
        if highest_bid:
            messages.info(request,f'Auction {auction.title} has ended. The winner is {highest_bid.user.username}')
        else:
            messages.info(request,f'Auction {auction.title} has ended with no bids.')
            utils.close_auction(auction.id)
    #actualizamos
    auctions = utils.get_auctions_by(is_active=True)
